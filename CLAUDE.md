...
`pseudoscript/` is a large worked-example model — the compiler modeling its own design, one `.pds` module per crate (bounded context). `examples/ticketing/` is a worked example: a surge-aware event-ticketing platform (virtual waiting room, atomic no-oversell holds, surge pricing, a hardened third-party payments saga). Both are buildable workspaces (`pds doc <dir>`).
