...
use pseudoscript_model::{Member, Symbol, Workspace, ast};
...
pub fn resolve_at(ws: &Workspace, from_fqn: &str, src: &str, offset: u32) -> Option<Hit> {
        ...
        .and_then(|entry| entry.model.member_at(clicked))
    {
        return Some(Hit {
            clicked,
            ..member_hit(member, owner, from_fqn.to_owned())
        });
    }
...
fn resolve_member(ws: &Workspace, from_fqn: &str, tokens: &[Token], idx: usize) -> Option<Hit> {
    ...
    Some(member_hit(member, &symbol.name, owner_module.to_owned()))
}
...
/// the callable's `///` summary when present.
fn member_hit(member: &Member, owner: &str, owner_module: String) -> Hit {
    let kind = match member.kind {
        pseudoscript_model::MemberKind::Callable => "callable",
        pseudoscript_model::MemberKind::Field => "field",
    };
...
pub(crate) fn module_of(fqn: &str) -> &str {
...
pub(crate) fn enclosing_node(module: &ast::Module, offset: u32) -> Option<String> {
