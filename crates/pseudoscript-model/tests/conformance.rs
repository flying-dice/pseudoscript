...
component Generate for Cli {
  ...
  public run(req: Request): Result<Diagram, Failure> {
    ast = Result<Ast, Failure> from Syntax::Parser.parse(req)
    if (ast.isErr) {
      return Err(ast.error)
    }
    graph = Result<Graph, Failure> from Model::Builder.build(ast.value)
    if (graph.isErr) {
      return Err(graph.error)
    }
    sub = Graph from Model::Views.extract(graph.value)
    out = Result<Diagram, Failure> from Emit::Transpiler.emit(sub)
    if (out.isErr) {
