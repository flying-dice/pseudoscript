...
**Status:** Accepted
**Supersedes:** ADR-004
**Affects:** LANG.md §2.3, §3.3, §5.1, §8.1, §9.2, §10
...
- `self` is not reserved (§2.3). It is an ordinary identifier with no special meaning; `self.Name(args)` parses as member access on an undeclared `self` and fails resolution like any unknown name. No `self`-specific diagnostic.
...
- Rejected alternative: keep `self` reserved to emit a migration diagnostic on the removed `self.` form. The value is transitional only, and it leaves a dead keyword in the grammar — every lexer, highlighter, and completion path carries a `self` case for a token no valid model uses. Pre-1.0, with no published models to migrate, the cost outweighs the guided error; `self` is freed as an ordinary identifier instead.
