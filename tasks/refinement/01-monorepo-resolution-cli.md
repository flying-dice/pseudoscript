...
ADR note: a path dep is a §8.3 dependency whose source is a local path instead of git; its identity has no commit, so §8.3 lockfile pinning does not apply. Worth an ADR amending §8.3–§8.3 (path source) — co-author with the spec-style skill.
...
- Docs: `LANG.md §8.3–§8.3` (path source clause), new `decisions/NNN-path-dependencies.md`, `PATTERNS.md` monorepo recipe, CLI tests under `crates/pseudoscript/tests/`.
...
- Strict vs fused monorepo graph: must a project declare a `[dependencies]` entry to see a sibling (strict, matches §8.3's explicit-root rule), or auto-resolve any sibling under the root (fused)? Recommend strict — preserves the §8.1/§8.3 "each root is declared" discipline.
...
- `pds check` (or `pds doc`) on project A resolves a reference to a `public` node in sibling project B via a path dep; a reference to a **private** B node is rejected (extends §8.2/§8.3).
