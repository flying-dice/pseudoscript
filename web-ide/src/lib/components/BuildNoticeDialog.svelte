<script lang="ts">
  ...
  // no folder to write to. Offers a read-only preview, an escape hatch to open a
  // real folder (when the File System Access API is available), or cancel. Built
  // on the shadcn Dialog for focus-trap, scroll-lock, and Escape handling.
  ...
  let { onopenfolder, onbuild, oncancel }: Props = $props();
...
<Dialog.Root open onOpenChange={onOpenChange}>
  <Dialog.Content class="sm:max-w-md" data-testid="build-notice">
    <Dialog.Header>
      <Dialog.Title>Build the example as a preview?</Dialog.Title>
    </Dialog.Header>
    <div class="prose">
      <p>
    ...
    <Dialog.Footer>
      ...
      {#if onopenfolder}
        <Button type="button" variant="outline" size="sm" onclick={() => onopenfolder?.()}>Open a folder…</Button>
      {/if}
      <Button type="button" size="sm" onclick={() => onbuild?.()}>Build preview</Button>
    </Dialog.Footer>
  </Dialog.Content>
</Dialog.Root>
